  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (line.trimStart().startsWith("- ")) {
      ...
      if (!Array.isArray(parent)) {
        die(`Invalid YAML structure near line ${i + 1}: list item without list parent`);
      }
      parent.push(item);
      continue;
    }
...
function main() {
  ...
  try { sh(`git rev-parse --verify ${base}`); } catch { die(`Base ref not found: ${base}`); }
  try { sh(`git rev-parse --verify ${head}`); } catch { die(`Head ref not found: ${head}`); }
  ...
  const diffLines = Object.values(numstats).reduce((s, v) => s + v.add + v.del, 0);

  console.log("policy-gate: target range", { base, head });
  console.log("policy-gate: stats", { filesChanged, newFiles, deletedFiles, diffLines });

  // Hard gate
  const hardHits = changes.filter((c) => anyMatch(hardGate, c.path)).map((c) => c.path);
  if (hardHits.length) {
  ...
  const configHits = (softGate.config || []).length
    ? changes.filter((c) => anyMatch(softGate.config, c.path)).map((c) => c.path)
    : [];

  if (depsHits.length && !allowDeps) die(`Soft gate (deps) hit without allow: ${depsHits.join(", ")}`);
  if (configHits.length && !allowConfig) die(`Soft gate (config) hit without allow: ${configHits.join(", ")}`);

  // Limits
  function over(name, actual, max) {
    return typeof max === "number" && max >= 0 && actual > max;
  }
  if (over("max_files_changed", filesChanged, limits.max_files_changed)) die(`Limit exceeded: filesChanged=${filesChanged}`);
  if (over("max_new_files", newFiles, limits.max_new_files)) die(`Limit exceeded: newFiles=${newFiles}`);
  if (over("max_deleted_files", deletedFiles, limits.max_deleted_files)) die(`Limit exceeded: deletedFiles=${deletedFiles}`);
  if (over("max_diff_lines", diffLines, limits.max_diff_lines)) die(`Limit exceeded: diffLines=${diffLines}`);
