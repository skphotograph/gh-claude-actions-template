...
### B-1. `/retry` が実際のリトライを行わない【中】 ✅ 対応済み（2026-02-20 / `fix/b1-retry-triggers-workflow`）
...
### B-3. CLAUDE.md のスケルトンがない【低〜中】 ✅ 対応済み（2026-02-20 / `fix/b3-add-claude-md-skeleton`）
...
### B-4. .gitignore が存在しない【低】 ✅ 対応済み（2026-02-20 / `fix/b4-add-gitignore-skeleton`）
...
### C-2. API コスト管理の仕組みがない【低〜中】 ✅ 対応済み（2026-02-19 / `fix/c2-add-max-tokens-guardrail`）
...
### D-2. `policy-gate.js` の `name-status` 解析が rename を壊す（Medium） ✅ 対応済み（2026-02-20 / `fix/d2-parse-rename-name-status`）
